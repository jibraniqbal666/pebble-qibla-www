...
class Malaysia(Timetable):
    ...
    def _mangleTime(cls, time_str, date, aft, maybe_morn):
        time = cls._parse_clock(time_str)
        if aft:
    ...
    def Times(cls, location, date):
        ...
        resp = requests.get(ESOLAT_API, params=params, timeout=60)
        ...
        for row in rows:
            ...
            results.append((zone.Name, this_date, {
                        ...
                        "dhuhr": cls._mangleTime(row["dhuhr"], this_date, True, True),
                        "asr": cls._mangleTime(row["asr"], this_date, True, False),
                        "maghrib": cls._mangleTime(row["maghrib"], this_date, True, False),
                        "isha": cls._mangleTime(row["isha"], this_date, True, False),
                        }
                    ))
        return results
